...
public class DBManager {
    ...
    public DBManager() {
        try {
            ...
            // khi mất dữ liệu thì thêm ! vào if rồi run App
            if (file.exists()) {
                return;
            } else {
    ...
    public boolean getNoteListFromDatabase() {
        ...
        while (!cursor.isAfterLast()) {
            ...
            noteArrayList.add(new Note(cursor.getInt(cursor.getColumnIndex("id")),
    ...
    public boolean deleteNote(Note note) {
        openDB();
        String[] strings = {String.valueOf(note.getId())};
        int row = sqLiteDatabase.delete(NOTE_TABLE, "id = :id", strings);
        closeDB();
        if (row > 0) {
            Log.d(TAG, "xoa on db");
